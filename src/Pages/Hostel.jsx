...
<div className='w-full flex flex-col items-center justify-center'>
  ...
  <div className='w-4/5 flex flex-col md:flex-row'>
    <div className='w-full md:w-1/2'>
      {images.map((image, index) => (
        <div key={index} style={{ display: activeIndex === index ? 'block' : 'none' }}>
          <img src={image.image} alt={image.title} className='w-full h-96 object-cover rounded-lg' />
        </div>
      ))}
      <div className='flex mt-4'>
        {images.map((image, index) => (
          <div key={index} onClick={() => handleImageClick(index)} className='w-1/5 md:w-1/6'>
            <img
              src={image.image}
              alt={image.title}
    ...
    <div className='w-full sm:pl-4 md:w-1/2 flex flex-col items-center'>
      <div>
